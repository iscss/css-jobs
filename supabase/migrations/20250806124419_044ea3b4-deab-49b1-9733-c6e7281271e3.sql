...
-- 1. Drop the insecure admin_user_profiles view and recreate as a security definer function
DROP VIEW IF EXISTS public.admin_user_profiles;

-- Create a secure function to get admin user profiles data
...
BEGIN
  ...
  -- Return the admin user profiles data
  RETURN QUERY
  SELECT 
    up.id,
    up.full_name,
...
CHECK (institution IS NULL OR length(institution) <= 200);
